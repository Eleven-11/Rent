  ...
  <div class="app-container">
    ...
    <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect">
      <el-submenu v-for="item in MenuParent" :key="item.postLabelId" :index="item.postLabelId">
        ...
        <el-menu-item :index="child.postLabelId" v-for="child in item.children" :key="child.postLabelId"
                      v-if="child.children!=null">{{child.labelContent}}
        </el-menu-item>
        <el-menu-item :index="item.postLabelId" v-for="(child,index) in item.children" :key="child.postLabelId"
                      v-if="child.children==null&&index<1">{{item.labelContent}}
        </el-menu-item>
    ...
    <el-table :data="tableData" v-loading.body="listLoading" element-loading-text="拼命加载中" border fit
      ...
      <el-table-column align="center" label="标签名" prop="labelContent" width="220"
                       style="width: 60px;"></el-table-column>
      <el-table-column align="center" label="英文" prop="english" width="220"
                       style="width: 60px;"></el-table-column>
    ...
    <el-dialog v-model="newPostLabel" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form class="small-space" :model="postLabel" label-position="left" label-width="80px"
        ...
        <el-form-item label="英文" style="margin-left: 10px" v-if="dialogStatus=='update'">
          <span style="margin-left: -10px">
          <el-input type="text"  v-model="postLabel.english">{{postLabel.english}}
          </el-input>
          </span>
        </el-form-item>
          ...
          function listToTree(postLabelId, labelParentId, list) {
            function exists(list, parentId) {
              for (var i = 0; i < list.length; i++) {
                ...
                if (list[i].postLabelId == parentId) {
                  return true;
                }
              }
              return false;
